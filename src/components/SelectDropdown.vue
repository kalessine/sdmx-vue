...
export default {
  setup(props: any) {
    ...
    const items = computed(() => {
      if (binding.value.clientSide || binding.value.boundTo == 20) {
        return store.state.visual.query
          .getQueryKey(props.concept)
          .getWalkedValues()
          .map((item: structure.ItemType) => {
            return {
              name: structure.NameableType.toString(item),
